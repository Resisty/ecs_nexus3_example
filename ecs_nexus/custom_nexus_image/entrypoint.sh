...
PRIVATE_KEY_PASSWORD="$(echo $(aws kms decrypt \
                               --region us-west-2 \
                               --ciphertext-blob fileb://<(echo -n "$PRIVATE_KEY_PASSWORD" | base64 --decode) \
                               --output text \
...
if [ "$1" == 'bin/nexus' ]; then
  if [ ! -f "$NEXUS_SSL/keystore.jks" ]; then
    ...
    if [ ! -f $PUBLIC_CERT ] && [ ! -f $PRIVATE_KEY ]; then
      openssl req -nodes -new -x509 -days 730 -keyout $PRIVATE_KEY -out $PUBLIC_CERT -subj "${PUBLIC_CERT_SUBJ}"
    fi
